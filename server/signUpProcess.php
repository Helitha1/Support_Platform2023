...
if (isset($_POST["json"])) {
    ...
    if (isset($request_obj->email) && isset($request_obj->fname) && isset($request_obj->lname) && isset($request_obj->password) && isset($request_obj->gender) && isset($request_obj->profession_id)) {
        ...
        if ($request_obj->fname=="") {
            ...
            $code = 6;
        } else if (!filter_var($erequest_obj->mail, FILTER_VALIDATE_EMAIL)) {
            $code = 7;
            ...
            if ($rs->num_rows > 0) {
                ...
                ('" . $request_obj->fname . "','" . $request_obj->lname . "','" . $request_obj->email . "','" . $password_hash . "','" . $request_obj->gender . "','" . $request_obj->date . "','" . $request_obj->profession_id . "')");
